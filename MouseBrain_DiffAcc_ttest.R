...
#3. Differential T-test with Original peaks
...
br_org_test2 <- br_org_test[which(! br_org_test$name2 %in% filter_genes), ] ## 16,451 (-11)
br_f10_test2 <- br_f10_test[which(! br_f10_test$name2 %in% filter_genes), ] ## 11,515 (-6)

na_org_test2 <- na_org_test[which(! na_org_test$name2 %in% filter_genes), ] ## 15,706 (-9)
na_f10_test2 <- na_f10_test[which(! na_f10_test$name2 %in% filter_genes), ] ## 10,434 (-5)
...
pdf("MouseBrain_nzd_reads_amyg_vs_cortex.pdf") ## Figure 5C
...
pdf("MouseBrain_nzd_reads_vs_pvalues.pdf") ## Supplementary Figure 
