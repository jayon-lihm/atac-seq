...
fwd_fastq=${file_prefix}_1.fastq.gz
rev_fastq=${file_prefix}_2.fastq.gz
...
java -jar ${trim_prog_path}trimmomatic-0.32.jar PE -trimlog ${file_prefix}_trimmomatic.log -baseout ${file_prefix}_trimmed ${fwd_fastq} ${rev_fastq} ILLUMINACLIP:${adapter}:2:30:10 LEADING:3 TRAILING:3 SLIDINGWINDOW:4:15 MINLEN:36
gzip -f ${file_prefix}_trimmomatic.log

##2. Map to mm9 (bowtie2)
echo "2. Map to mm9 (bowtie2)"
fwd_fastq="${file_prefix}_trimmed_1P"
rev_fastq="${file_prefix}_trimmed_2P"
bamfile="${file_prefix}_trimmed"

## parameter set as in Denny et al
bowtie2 -X 2000 -x ${bowt_mm9_path}mm9 -1 $fwd_fastq -2 $rev_fastq | samtools view -bhS - > ${bamfile}.bam
